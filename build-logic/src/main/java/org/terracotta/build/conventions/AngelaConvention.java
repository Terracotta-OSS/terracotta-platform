...
public class AngelaConvention implements ConventionPlugin<Project, AngelaPlugin> {
  ...
  public void apply(Project project) {
    ...
    ModuleDependency angela = dependencyFactory.create("org.terracotta", "angela", project.property("angelaVersion").toString())
            .exclude(group("org.slf4j")).exclude(group("javax.cache"));
    ModuleDependency angelaResolver = dependencyFactory.create(project.project(":common:angela-test-kit-resolver"))
            .exclude(group("org.slf4j")).exclude(group("javax.cache"));

    project.getConfigurations().named(FRAMEWORK_CONFIGURATION_NAME, config -> {
      config.getDependencies().add(angela);
      config.getDependencies().add(angelaResolver);
    });

    project.getConfigurations().named("angelaKit", config -> {
      ProjectDependency projectDependency = dependencyFactory.create(project.project(":terracotta"));
