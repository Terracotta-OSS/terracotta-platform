...
public class BaseConvention implements ConventionPlugin<Project, BasePlugin> {
  ...
  public static String toSagVersion(StructuredVersion v) {
    // convert 1.2.3.4.5 to 1.2.3.0004-0005, snapshot versions are unchanged.
    // this is needed for iBit (and preserves format from maven days)
    try {
      return String.format("%s.%s.%s.%04d-%04d",
          v.getAt(0), v.getAt(1), v.getAt(2),
          parseInt(v.getAt(3)), parseInt(v.getAt(4)));
