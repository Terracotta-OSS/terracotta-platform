...
public class HelloWorldTest {
  ...
  public void test_hello_world_management() throws Exception {
    try (Connection connection = ConnectionFactory.connect(URI.create("passthrough://stripe-1:9510/cluster-1"), new Properties())) {
      ...
      TmsAgentEntityFactory entityFactory = new TmsAgentEntityFactory(connection, getClass().getSimpleName());
      TmsAgentEntity consumerEntity = entityFactory.retrieveOrCreate(new TmsAgentConfig());

      ManagementRegistry registry = consumerEntity.readTopology().get()
          .clientStream()
