 ...
 */
class ActiveNmsServerEntity extends ActiveProxiedServerEntity<Nms, Void, Void, NmsMessenger> {

  private final AtomicBoolean connected = new AtomicBoolean();
  private final ActiveNms activeNms;

  ActiveNmsServerEntity(ActiveNms activeNms) {
    super(activeNms, activeNms);
  ...
  public void connected(ClientDescriptor clientDescriptor) {
    ...
    if (!connected.compareAndSet(false, true)) {
      throw new AssertionError("Only one connection allowed per " + NmsConfig.ENTITY_TYPE);
    }
  }
