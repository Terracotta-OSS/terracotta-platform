...
public class HelloWorldEntityFactory {
  ...
  public HelloWorldEntity create(final String identifier) throws EntityAlreadyExistsException {
    EntityRef<HelloWorldEntity, Void> ref = getEntityRef(identifier);
    try {
      ref.create(null);
  ...
  private EntityRef<HelloWorldEntity, Void> getEntityRef(String entityName) {
    try {
      return connection.getEntityRef(HelloWorldEntity.class, 1, entityName);
