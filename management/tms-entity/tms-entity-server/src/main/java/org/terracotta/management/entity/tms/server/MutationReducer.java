...
class MutationReducer {
  ...
  public MutationReducer(IMonitoringConsumer consumer, Cluster cluster, List<Notification> notifications) {
    ...
    this.clientConnectionNotifications = notifications.stream()
        .filter(notification -> notification.isAnyType(PlatformNotificationType.CONNECTION_OPENED, PlatformNotificationType.CONNECTION_CLOSED))
  ...
  public void reduce() {
    ...
    for (List<Notification> connectionNotifications : clientConnectionNotifications.values()) {
      for (int i = 0, max = connectionNotifications.size(); i < max; i++) {
        ...
        if (i == max - 1 && clientConnectionCount == 0 && notification.isAnyType(PlatformNotificationType.CONNECTION_CLOSED)) {
          ...
          connectionNotifications.stream()
              .limit(i)
              .filter(n -> n.isAnyType(PlatformNotificationType.CONNECTION_OPENED))
              .findFirst()
              .ifPresent(openedNotif -> insertAfter(openedNotif, "CLIENT_CONNECTED", clientContext));

        } else if (i == max - 1 && clientConnectionCount == 1 && notification.isAnyType(PlatformNotificationType.CONNECTION_OPENED)) {
