...
public class ClusterTopologyMBean extends AbstractTerracottaMBean {
  private static final Logger LOGGER = LoggerFactory.getLogger(ClusterTopologyMBean.class);
  private static final String MBEAN_NAME = "ClusterTopologyMBean";
  private final Cluster cluster;

  public static void init(Cluster cluster) {
    try {
      ClusterTopologyMBean clusterTopologyMBean = new ClusterTopologyMBean(cluster);
      clusterTopologyMBean.register();
    } catch (Exception e) {
      throw new RuntimeException("Failed to register " + MBEAN_NAME);
    }
  }

  private ClusterTopologyMBean(Cluster cluster) throws Exception {
