...
public class DataDirectories {
  ...
  public DataDirectories(Map<String, Path> dataDirs, Path nodeMetadataDir) {
    ...
    try {
      ...
      if (dataDirs != null) {
        for (Map.Entry<String, Path> entry : dataDirs.entrySet()) {
          ...
          dataDirectories.getDirectory().add(createDataRootMapping(entry.getKey(), entry.getValue()
                                                                                        .toString(), isPlatformRoot));
        }
      }

      if (nodeMetadataDir != null && !metadataRootFound) {
        dataDirectories.getDirectory()
                       .add(createDataRootMapping(META_DATA_ROOT_NAME, nodeMetadataDir.toString(), true));
      }
