...
public class AutoActivateNewPassiveIT extends DynamicConfigIT {
  ...
  public void test_auto_activation_failure_for_2x1_cluster() throws Exception {
    try {
      startNode(1, 1,
          ...
          "--node-repository-dir", "repository/stripe1/node1-1");
      fail();
    } catch (Exception e) {
  ...
  public void test_auto_activation_success_for_1x1_cluster() throws Exception {
    ...
    startNode(1, 1, "-f", configurationFile.toString(), "-s", "localhost", "-p", String.valueOf(getNodePort()), "--node-repository-dir", "repository/stripe1/node1-1");
  ...
  public void test_auto_activation_success_for_1x2_cluster() throws Exception {
    ...
    startNode(1, 1, "-f", configurationFile.toString(), "-s", "localhost", "-p", String.valueOf(getNodePort(1, 1)), "--node-repository-dir", "repository/stripe1/node1-1");
    waitUntil(out::getLog, containsString("Moved to State[ ACTIVE-COORDINATOR ]"));

    startNode(1, 2, "-f", configurationFile.toString(), "-s", "localhost", "-p", String.valueOf(getNodePort(1, 2)), "--node-repository-dir", "repository/stripe1/node1-2");
