...
public class ConfigSyncIT extends BaseStartupIT {
  ...
  public void testPassiveZapsWhenActiveHasSomeUnCommittedChanges() throws Exception {
    ...
    try {
      out.clearLog();
      ConfigTool.start("set", "-s", "localhost:" + shape.activePort, "-c", "stripe.1.node." + shape.activeId + ".tc-properties.org.terracotta.dynamic-config.simulate=commit-failure");
  ...
  public void testPassiveCanSyncAndRepairIfLatestChangeNotCommitted() throws Exception {
    ...
    try {
      ConfigTool.start("set", "-s", "localhost:" + shape.passivePort, "-c", "stripe.1.node." + shape.passiveId + ".tc-properties.org.terracotta.dynamic-config.simulate=recover-needed");
      fail("Expected to throw exception");
    } catch (IllegalStateException e) {
      e.printStackTrace(System.out);
    }
