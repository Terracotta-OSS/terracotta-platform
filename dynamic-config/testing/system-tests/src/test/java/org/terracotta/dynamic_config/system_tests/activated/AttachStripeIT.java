...
public class AttachStripeIT extends DynamicConfigIT {
  ...
  public void test_attach_stripe_to_activated_cluster() throws Exception {
    ...
    assertThat(getRuntimeCluster("localhost", getNodePort(1, 1)).getNodeCount(), is(equalTo(4)));
    assertThat(getUpcomingCluster("localhost", getNodePort(2, 1)).getNodeCount(), is(equalTo(4)));
    assertThat(getRuntimeCluster("localhost", getNodePort(2, 1)).getNodeCount(), is(equalTo(4)));
    ...
    assertThat(getRuntimeCluster("localhost", getNodePort(1, 1)).getStripe(2).get().getNodeCount(), is(equalTo(2)));
    assertThat(getUpcomingCluster("localhost", getNodePort(2, 1)).getStripe(1).get().getNodeCount(), is(equalTo(2)));
    assertThat(getRuntimeCluster("localhost", getNodePort(2, 1)).getStripe(1).get().getNodeCount(), is(equalTo(2)));
  }
}
