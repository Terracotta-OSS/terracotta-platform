...
public class SimulationHandlerIT extends DynamicConfigIT {
  ...
  public void test_commit_fails_permanently() {
    assertThat(
        ...
        allOf(
            not(hasExitStatus(0)),
            containsOutput("Reason: Error when applying setting change: 'set logger-overrides.org.terracotta.dynamic-config.simulate=INFO (stripe ID: 1, node: node-1-1)': Simulate permanent commit failure")));
  }
  ...
  public void test_commit_fails_temporary() {
    assertThat(
        ...
        allOf(
            not(hasExitStatus(0)),
            containsOutput("Reason: Error when applying setting change: 'set logger-overrides.org.terracotta.dynamic-config.simulate=DEBUG (stripe ID: 1, node: node-1-1)': Simulate temporary commit failure")));
  }
}
