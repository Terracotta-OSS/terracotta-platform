...
public class SimulationHandlerIT extends BaseStartupIT {
  ...
  public void test_missing_value() {
    ...
    exception.expectMessage(containsString("Invalid input: 'stripe.1.node.1.tc-properties.com.terracottatech.dynamic-config.simulate='. Reason: Operation set requires a value"));
    ConfigTool.start("set", "-s", "localhost:" + ports.getPort(), "-c", "stripe.1.node.1.tc-properties.com.terracottatech.dynamic-config.simulate=");
  }

  @Test
  public void test_prepare_fails() {
    exception.expect(IllegalStateException.class);
    exception.expectMessage(containsString("Prepare rejected for node localhost:" + ports.getPort() + ". Reason: Error when trying to apply setting change 'set tc-properties.com.terracottatech.dynamic-config.simulate=prepare-failure (stripe ID: 1, node: node-1)': Simulate prepare failure"));
  ...
  public void test_commit_fails() {
    exception.expect(IllegalStateException.class);
    exception.expectMessage(containsString("Commit failed for node localhost:" + ports.getPort() + ". Reason: com.terracottatech.nomad.server.NomadException: Error when applying setting change 'set tc-properties.com.terracottatech.dynamic-config.simulate=commit-failure (stripe ID: 1, node: node-1)': Simulate commit failure"));
