  ...
  public void test_getPendingTopology() throws Exception {
    try (DiagnosticService diagnosticService = DiagnosticServiceFactory.fetch(
      ...
      assertThat(pendingTopology, is(equalTo(new Cluster(new Stripe(new Node()
          ...
          .setNodeConfigDir(Env.getModulePath().resolve("../config"))
          .setNodeMetadataDir(Env.getModulePath().resolve("../metadata"))
          .setNodeLogDir(Env.getModulePath().resolve("../logs"))
          .setNodeBackupDir(Env.getModulePath().resolve("../backup"))
